          ...
          {sorted.map((p, idx) => {
            ...
            return (
              <button key={p.id} onClick={() => onSelectProject(p.id)}
                style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: isCurrent ? "12px 16px" : "9px 14px", background: isCurrent ? GREEN_LIGHT : C.bg, border: `${isCurrent ? 2 : 1}px solid ${isCurrent ? GREEN + "44" : C.border}`, borderRadius: 10, cursor: "pointer", textAlign: "left", transition: "all 0.15s" }}
